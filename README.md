...
- **AI story improvement** (vision-doc slice 7.7): in the board work-item detail panel, "Mejorar historia con IA" proposes a rewritten user story (`Como … quiero … para …`) plus acceptance criteria for an existing task. You preview it, then Aplicar replaces the description (the title is untouched) and adds the criteria to the edit form, or Descartar leaves everything as is; nothing is saved until you save the panel. Same opt-in flag as the other AI features.
- **AI risk analysis** (vision-doc slice 7.6): the project dashboard has a "Riesgos (IA)" card. "Analizar riesgos" detects concrete signals from your data (oversized stories, overloaded members, sprints at risk or overdue, stalled tasks, unassigned urgent tasks) and the assistant adds a summary plus recommendations. Nothing is stored; with no signals there is no AI call.
- **AI project creation** (vision-doc slice 7.4): on `/projects`, "Crear con IA" opens a two-step screen. Describe the project in free text and the assistant proposes a name, description, technologies and a backlog of epics with stories. You edit everything (rename, add/remove epics and stories, optional project code) and confirm: the project and its backlog are created atomically in one request, epics as parent tasks and stories as their subtasks in the first board column, and you land on the new board. Same opt-in flag as the other AI features; nothing is saved until you confirm.
...
See [`CLAUDE.md`](CLAUDE.md) for the full current-status breakdown and what remains toward the full product vision (vision-doc slice 7.5: an AI project manager).
...
All six AI features — user stories (7.1), subtasks (7.3), acceptance criteria (7.2), story improvement (7.7), risk analysis (7.6), and project creation (7.4) — share one opt-in flag. The default `docker compose up --build` runs with `flowpilot.ai.enabled=false` and a deterministic stub generator, so every AI endpoint and screen works without a model — the stub just returns a canned draft.

...
See [`CLAUDE.md`](CLAUDE.md) for the full current-status breakdown and what remains toward the full product vision (vision-doc slices 7.4–7.5: AI project creation and an AI project manager).
...
All five AI features — user stories (7.1), subtasks (7.3), acceptance criteria (7.2), story improvement (7.7), and risk analysis (7.6) — share one opt-in flag. The default `docker compose up --build` runs with `flowpilot.ai.enabled=false` and a deterministic stub generator, so every AI endpoint and screen works without a model — the stub just returns a canned draft.

...
public class CommentService {
 ...
 @Transactional public CommentResponse update(Long id,CommentUpdateRequest request,Long userId){Comment c=comments.findById(id).orElseThrow(()->new IllegalArgumentException("Comment not found: "+id)); requireView(userId,c.getProjectId()); if(!c.getAuthorId().equals(userId)) throw new AccessDeniedException("Only the comment author may edit it"); c.updateContent(request.content()); activity.record(c.getProjectId(),userId,ActivityEventType.COMMENT_UPDATED,"Comment updated","{\"commentId\":"+id+"}"); return response(c);}

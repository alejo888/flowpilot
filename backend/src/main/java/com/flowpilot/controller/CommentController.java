...
public class CommentController {
 ...
 @GetMapping("/api/projects/{projectId}/comments") public List<CommentResponse> projectComments(@PathVariable Long projectId,@RequestParam(defaultValue="20") @Min(1) @Max(100) int limit,@RequestParam(defaultValue="0") @Min(0) int offset,Authentication a){return comments.listProject(projectId,id(a),limit,offset);}
 ...
 @GetMapping("/api/work-items/{workItemId}/comments") public List<CommentResponse> workItemComments(@PathVariable Long workItemId,@RequestParam(defaultValue="20") @Min(1) @Max(100) int limit,@RequestParam(defaultValue="0") @Min(0) int offset,Authentication a){return comments.listWorkItem(workItemId,id(a),limit,offset);}
 ...
 @GetMapping("/api/projects/{projectId}/activity") public List<ActivityResponse> activity(@PathVariable Long projectId,@RequestParam(defaultValue="20") @Min(1) @Max(100) int limit,@RequestParam(defaultValue="0") @Min(0) int offset,Authentication a){if(!auth.canView(id(a),projectId)) throw new org.springframework.security.access.AccessDeniedException("Not authorized to view project");return activity.list(projectId,limit,offset);}

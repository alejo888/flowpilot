...
- Comments and project activity feed: backend supports project-scoped and work-item-scoped comments (create/read/update/delete, delete author-only like update, records a `COMMENT_DELETED` activity event) plus an authorization-checked project activity feed (`CommentController`, `ProjectActivityService`); frontend renders both in the project-detail screen's comments and activity sections, plus in the board's work-item detail panel, with an accessible delete confirmation dialog for comments the caller authored. Each comment's author/date line and its Editar/Eliminar action row are separate flex rows (`.comment-meta`, `.comment-actions`) inside a `display:flex; flex-direction:column` article — needed because the article previously had no layout of its own, so the author/date text ran together and, while editing, the textarea visually overlapped the line above it.
...
- E2E coverage (`frontend/e2e/`) now also runs an axe-core WCAG 2.1 A/AA accessibility check (`accessibility.*.spec.ts`, `assert-no-a11y-violations.ts`) across every authenticated static route including `/projects/:id/dashboard` and `/projects/:id/backlog`, plus board/members, and the guest-only specs (no login required) run cross-browser in `firefox-guest`/`webkit-guest` Playwright projects in addition to the existing chromium ones. Authenticated and desktop specs stay chromium-only to avoid the refresh-token reuse-detection concurrency issue already documented in `playwright.config.ts`. Still not a general cross-browser suite for authenticated flows.
